...
class AvellanedaStoikovModel:
    ...
    def get_quotes(self, mid_price, inventory):
        """
        ...
        reservation_price = mid_price - (inventory * self.risk_aversion * (self.volatility ** 2) * self.terminal_time)
        
        # 2. Optimal Spread
        # delta = gamma * sigma^2 * T + (2/gamma) * ln(1 + gamma/k)
        vol_term = self.risk_aversion * (self.volatility ** 2) * self.terminal_time
